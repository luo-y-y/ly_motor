(function($) {
	...
	var optionDefaults = {
		...
		//所有天气状态,几乎所有的天气都可以归结为这五种
		weatherStates : {
			sunshine : { length : 1 },//晴天 /* typesNum , child  */
			cloudy : { length : 1 },//多云
			overcast : { length : 2 },//阴天
			rainy : { length : 2, drop : [{ time : 300 }, { time : 300 }] },// 雨
			sleet : { length : 2, drop : [{ time : 300 }, { time : 800 }] },//雨夹雪
			snow : { length : 2, drop : [{ time : 800 }, { time : 800 }] }//雪
		}
	};
	...
	$.fn.weather = function(option) {
		...
		return obj.$weatherArea;
		//返回天气预报的区域 , .weather-info
	};
	...
	function Weather($element, opts) {
		...
		this.sky = opts.sky;
		// 天气
		this.skyState = opts.weatherStates[opts.sky];
		//天气状态信息
	};
	...
	Weather.prototype = {
		...
		init : function(config) {
			...
			html = [],
			//当前天气子节点数量
			skyState_child_length = (this.skyState || '' ).length, templeteHtml = 
				'<div style="display:none;" class="weather-info">'+
					'<h3>{#city#}<span>{#date#}</span></h3>'+
					...
					'<div class="night_weather">'+
						'<span class="weather">{#night#} : {#night_weather#}</span>/<span class="temp">{#night_temp#}{#temp#}</span>'+
					'</div>'+
				'</div>', i = 0;
		...
		moveIcon : function() {
			...
			this.$weather.each(function(i) {
				...
				$this.css({
					left : rand(0, $this.parent().width() * 0.8)
				}).show();
				$this.hover(function() {
					$this.stop();
					if (myTime[i])
				...
				$(window).resize(function() {
					...
					clearTimeout(myTime[i] || {});
					$this.stop().animate({
						left : rand(0, $this.parent().width() * 0.5)
					}, 300, function() {
						creatMove($this, i);
					});
				});
			...
			function creatMove($this, i) {
				var current_function = arguments.callee, widthMax = $this.parent().width(), start = 0, end = 0, width = $this.width() || 160, leftMax = widthMax - width, duration = _self.opts.duration, left = rand(0, leftMax), fps = _self.opts.fps, speed = Math.abs(parseInt($this.css('left')) - left) * rand(fps * 0.8, fps * 1.2) || 5000, time = rand(duration - duration * 0.5, duration + duration * 0.5);
				$this.animate({
					left : left
		...
		creatDrop : function() {
			...
			this.$weather.each(function(i) {
				...
				if (!_self.skyState.drop || !_self.skyState.drop.length)
					return false;
				var $this = $(this), $child = $this.find('.child'), dropTime = null;
		...
		drop : function($wearherChild) {
			...
			$wearherChild.find('div[name=init]').each(function() {
				var $this = $(this), top = parseInt($this.css('top')) || 0, flyTime = parseInt($this.attr('data-fly-time')) || 200;
				$this.attr({
					name : 'run'
				});
				...
				setTimeout(function() {
					$this.css({
						display : 'block',
						top : rand(top - 10, top + 10)
					}).animate({
						'top' : $wearherChild.height() + parseInt($this.attr('data-end')),
						opacity : 0
					}, flyTime, function() {
						$this.remove();
					});
		...
		makeDrop : function($wearherChild) {
			var objWidth = $wearherChild.width(), objHeight = $wearherChild.height(), left = 0, end = 0, index = rand(0, (this.skyState.drop.length || 1 ) - 1), flyTime = this.skyState.drop[index].time || 300, is = '', i = 0, i_len = rand(2, 6);
